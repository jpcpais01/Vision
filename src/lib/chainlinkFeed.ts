 ...
 * directly from it. What is *not* independently verified — this sandbox has
 * no route to polymarket.com — is that the endpoint behaves the same for a
 * browser connection as it does for that Node client, and exactly how the
 * server responds to the ping. Both failure modes degrade safely: this is one
 * of three ways the barrier gets captured (see `Engine.captureBarrier`), and
 * the other two do not depend on it.
 */
